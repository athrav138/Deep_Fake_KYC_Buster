...
if (process.env.FIREBASE_SERVICE_ACCOUNT) {
  try {
    ...
    } else if (saValue !== "" && !saValue.includes("YOUR_")) {
      // If it's not a JSON and not a placeholder, it might be a file path (not supported here)
      console.warn("FIREBASE_SERVICE_ACCOUNT is set but does not appear to be a JSON string. Initialization skipped.");
    }
  } catch (err: any) {
    console.error("Failed to initialize Firebase Admin:", err.message);
    console.info("Please ensure FIREBASE_SERVICE_ACCOUNT is a valid JSON string of your service account key.");
  }
}
...
function initDb() {
  ...
  if (!userColumns.includes('role')) {
    console.log("Migrating users: adding role column");
