...
export default function HistoryPage({ user }: { user: any }) {
  ...
  const fetchHistory = async () => {
    try {
      ...
      let firestoreVideoData: any[] = [];
      if (db && user?.id) {
        try {
          // Fetch KYC records (if any were stored in Firestore - though previous code was mostly RTDB)
          // For now let's assume we want to fetch from our new Firestore paths
          
          // Fetch Video records from the new Firestore path
          const videoPath = `video_analyses/${user.id}/results`;
          const videoSnapshot = await getDocs(collection(db, videoPath));
          firestoreVideoData = videoSnapshot.docs.map(doc => ({
            id: doc.id,
            ...doc.data(),
            is_deepfake: (doc.data() as any).isDeepfake,
            risk_level: (doc.data() as any).riskLevel,
            confidence_score: (doc.data() as any).confidenceScore,
            video_name: (doc.data() as any).videoName,
            analysis_data: (doc.data() as any).analysisData,
            created_at: (doc.data() as any).createdAt || new Date().toISOString(),
            isFirestore: true
          }));

        } catch (fbErr) {
          handleFirestoreError(fbErr, OperationType.GET, `video_analyses/${user.id}/results`);
        }
      }

      // Merge and sort
      const allKyc = [...kycData].sort((a, b) => 
        new Date(b.created_at || b.createdAt).getTime() - new Date(a.created_at || a.createdAt).getTime()
      );

      const allVideo = [...firestoreVideoData, ...videoData].sort((a, b) =>
        new Date(b.created_at || b.createdAt).getTime() - new Date(a.created_at || a.createdAt).getTime()
      );
      
      setKycRecords(allKyc);
      setVideoRecords(allVideo);
    } catch (err) {
