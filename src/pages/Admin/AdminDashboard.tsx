...
export default function AdminDashboard() {
  ...
  const fetchData = async () => {
    try {
      ...
      if (db) {
        try {
          // We need to fetch all users and their KYC submissions
          // This might be expensive, but for admin dashboard we usually need it.
          // Note: In a real app we might use a collectionGroup or a flatter structure.
          
          const usersSnapshot = await getDocs(collection(db, 'users'));
          const usersList = usersSnapshot.docs.map(doc => ({ id: doc.id, ...doc.data() }));

          for (const userDoc of usersList) {
             const submissionsSnapshot = await getDocs(collection(db, `kyc_users/${userDoc.id}/submissions`));
             submissionsSnapshot.docs.forEach(doc => {
               const record = doc.data();
               firestoreUsers.push({
                 id: doc.id,
                 ...record,
                 full_name: (record as any).name,
                 status: (record as any).verificationStatus?.toLowerCase() || 'verified',
                 risk_score: (record as any).riskScore,
                 kyc_date: (record as any).createdAt || new Date().toISOString(),
                 isFirestore: true
               });
             });
          }
        } catch (fbErr) {
          handleFirestoreError(fbErr, OperationType.LIST, 'users/kyc_users');
        }
      }
      ...
      const updatedStats = {
        ...
        stats: statsData?.stats ? {
          ...
          verified: (statsData.stats.verified || 0) + firestoreUsers.filter(u => u.status === 'accepted' || u.status === 'verified' || u.status === 'approved').length,
          fake: (statsData.stats.fake || 0) + firestoreUsers.filter(u => u.status === 'rejected' || u.status === 'fake').length,
        } : {
          total: firestoreUsers.length,
          verified: firestoreUsers.filter(u => u.status === 'accepted' || u.status === 'verified' || u.status === 'approved').length,
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-12">
        <div className="bg-app-card p-4 rounded-2xl border border-app-border flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className={cn("w-2 h-2 rounded-full", process.env.GEMINI_API_KEY ? "bg-emerald-500" : "bg-red-500")} />
            <span className="text-xs font-bold uppercase tracking-widest opacity-60">Gemini AI API</span>
          </div>
          <span className="text-[10px] font-mono opacity-40">{process.env.GEMINI_API_KEY ? 'CONFIGURED' : 'MISSING'}</span>
        </div>
        <div className="bg-app-card p-4 rounded-2xl border border-app-border flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className={cn("w-2 h-2 rounded-full", db ? "bg-emerald-500" : "bg-red-500")} />
            <span className="text-xs font-bold uppercase tracking-widest opacity-60">Firestore Database</span>
          </div>
          <span className="text-[10px] font-mono opacity-40">{db ? 'CONNECTED' : 'OFFLINE'}</span>
        </div>
        <div className="bg-app-card p-4 rounded-2xl border border-app-border flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className={cn("w-2 h-2 rounded-full", auth ? "bg-emerald-500" : "bg-red-500")} />
            <span className="text-xs font-bold uppercase tracking-widest opacity-60">Firebase Auth</span>
          </div>
          <span className="text-[10px] font-mono opacity-40">{auth ? 'READY' : 'ERROR'}</span>
        </div>
      </div>
