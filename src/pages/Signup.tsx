    ...
    try {
      ...
      if (db) {
        try {
          ...
          handleFirestoreError(fbErr, OperationType.CREATE, `users/${user.uid}`);
        }
      }
    ...
    try {
      ...
      try {
        await safeFetch('/api/auth/signup', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ email: trimmedEmail, password: validatedData.password, fullName: validatedData.fullName }),
        });
      } catch (err: any) {
        if (err.message.includes("already exists")) {
      ...
      if (auth && db) {
        try {
          ...
          handleFirestoreError(fbErr, OperationType.CREATE, `users/${auth.currentUser?.uid || 'new_user'}`);
            ...
            <div className="space-y-2">
              ...
              <div className="relative">
                ...
                <input 
                  ...
                  className={cn(
                    "w-full bg-app-bg border rounded-xl py-3 pl-12 pr-4 focus:outline-none focus:ring-2 transition-all",
                    fieldErrors.fullName ? "border-red-500/50 focus:ring-red-500/50" : "border-app-border focus:ring-emerald-500/50"
                  )}
                  placeholder="John Doe"
                />
              </div>
              {fieldErrors.fullName && <p className="text-red-400 text-[10px] ml-1 uppercase tracking-widest font-bold">{fieldErrors.fullName}</p>}
            </div>
            ...
            <div className="space-y-2">
              ...
              <div className="relative">
                ...
                <input 
                  ...
                  className={cn(
                    "w-full bg-app-bg border rounded-xl py-3 pl-12 pr-4 focus:outline-none focus:ring-2 transition-all",
                    fieldErrors.email ? "border-red-500/50 focus:ring-red-500/50" : "border-app-border focus:ring-emerald-500/50"
                  )}
                  placeholder="name@company.com"
                />
              </div>
              {fieldErrors.email && <p className="text-red-400 text-[10px] ml-1 uppercase tracking-widest font-bold">{fieldErrors.email}</p>}
            </div>
            ...
            <div className="space-y-2">
              ...
              <div className="relative">
                ...
                <input 
                  ...
                  className={cn(
                    "w-full bg-app-bg border rounded-xl py-3 pl-12 pr-4 focus:outline-none focus:ring-2 transition-all",
                    fieldErrors.password ? "border-red-500/50 focus:ring-red-500/50" : "border-app-border focus:ring-emerald-500/50"
                  )}
                  placeholder="••••••••"
                />
              </div>
              {fieldErrors.password && <p className="text-red-400 text-[10px] ml-1 uppercase tracking-widest font-bold">{fieldErrors.password}</p>}
            </div>
