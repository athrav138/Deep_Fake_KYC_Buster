...
export default function VideoAnalysis() {
  ...
  const analyzeVideo = async () => {
    ...
    try {
      ...
      // 1. Save to SQLite
      persistencePromises.push(safeFetch('/api/video/analyze', {
        method: 'POST',
        headers: {
      ...
      if (db && auth.currentUser) {
        const finalPath = `video_analyses/${auth.currentUser.uid}/results`;
        try {
          persistencePromises.push(addDoc(collection(db, finalPath), {
            videoName: videoFile.name,
            isDeepfake: data.isDeepfake,
            riskLevel: data.riskLevel,
            confidenceScore: data.confidenceScore,
          ...
          handleFirestoreError(fbErr, OperationType.CREATE, finalPath);
        }
      }

      await Promise.all(persistencePromises).catch(err => {
        console.warn("One or more persistence tasks failed:", err);
      });
              ...
              >
                ...
                <div className={cn(
                  ...
                  <div>
                    ...
                    <p className="opacity-60 text-sm mt-2 max-w-sm">{result.reasoning || result.explanation}</p>
                  </div>
                </div>
                ...
                <div className="bg-app-card p-8 rounded-[40px] border border-app-border">
                  ...
                  <div className="space-y-3">
                    {(result.anomalies || result.detectedAnomalies || []).map((anomaly: string, i: number) => (
