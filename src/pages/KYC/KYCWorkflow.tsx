...
export default function KYCWorkflow({ user }: { user: any }) {
  ...
  const [analysisStatus, setAnalysisStatus] = useState('Processing Biometric Data...');
  ...
  useEffect(() => {
    ...
    if (step === 'analysis' && !error) {
      const statuses = [
        ...
        'Detecting Liveness Patterns...',
        'Verifying Voice Authenticity...',
        'Running Deepfake Neural Networks...',
        'Cross-referencing ID Documents...',
  ...
  useEffect(() => {
    if (db && user?.id) {
      const historyPath = `kyc_users/${user.id}/submissions`;
      const unsubscribe = onSnapshot(collection(db, historyPath), (snapshot) => {
  ...
  useEffect(() => {
    const apiKey = process.env.GEMINI_API_KEY;
    if (!apiKey) {
      console.warn("GEMINI_API_KEY is missing in Secrets panel.");
    }
    if (!db) {
      console.warn("Firestore Database is not configured.");
    }
  }, []);
  ...
  const startVoiceRecording = async () => {
    ...
    try {
      ...
      if (SpeechRecognition) {
        ...
        setVoiceTranscript('(Live transcript not supported in this browser, but audio is being recorded)');
      }
  ...
  const verifyVoice = async () => {
    ...
    // Set step to analysis immediately for faster UI feedback
    setStep('analysis');
    ...
    try {
      ...
      const response = await ai.models.generateContent({
        ...
        contents: [
          {
            parts: [
              { text: `Perform a comprehensive KYC final analysis. 
              
              Context:
              1. Aadhaar Analysis: ${JSON.stringify(aadhaarResult)}
              2. Face Liveness Analysis: ${JSON.stringify(faceResult)}
              3. Voice Audio Provided: The user was instructed to say "${expectedText}" (Code: ${verificationCode}).

              Tasks:
              - Voice Analysis: Transcribe the audio. Verify if the code matches. Detect deepfake/synthetic voice artifacts.
              - Final Decision: Synthesize Aadhaar, Face, and Voice results into a final verification status.
              - Multi-Biometric Synthesis: Calculate a final risk score (0-100) and confidence score (0-100).
              ...
              { 
                ...
                finalResult: {
                  decision: "verified" | "suspicious" | "fake",
                  riskScore: number,
      ...
      if (data.voiceResult?.transcript) setVoiceTranscript(data.voiceResult.transcript);
      setVoiceResult(data.voiceResult);
      ...
      // Parallelize all data persistence for speed
      ...
      if (db && user?.id) {
        ...
        try {
          const kycData = {
            userId: user.id,
            name: personalDetails.fullName || user.fullName || 'unknown',
            ...
            voiceResult: data.voiceResult,
            aadhaarNumber: aadhaarResult?.aadhaarNumber || 'N/A'
          };
          
          persistencePromises.push(addDoc(collection(db, historyPath), kycData));
          
          // Update user's general status in Firestore
          persistencePromises.push(setDoc(doc(db, "users", user.id), {
            kycStatus: final.decision,
            lastKycAt: new Date().toISOString()
          }, { merge: true }));
        } catch (fbErr) {
          handleFirestoreError(fbErr, OperationType.WRITE, `kyc_users/${user.id}`);
        }
      }

      // 2. Save to local DB with all details
      persistencePromises.push(safeFetch('/api/kyc/finalize', {
        method: 'POST',
        headers: { 
      ...
      // Wait for all persistence to complete in parallel
      await Promise.all(persistencePromises).catch(err => {
        console.warn("One or more persistence tasks failed, but continuing to show result:", err);
      });
      
      showToast('KYC process completed successfully', 'success');
      setStep('result');
    } catch (err: any) {
      console.error("Voice & Finalization Error:", err);
      const msg = getFriendlyErrorMessage(err) || 'Verification failed';
      setError(msg);
      showToast(msg, 'error');
      // If error, stay on analysis step so user can see error and retry
    } finally {
      setLoading(false);
    }
  };

  // finalizeKYC is now redundant as its logic is merged into verifyVoice
  const finalizeKYC = async (vResult?: any) => {
    // Keep this as a no-op if called anywhere else, but its logic is now in verifyVoice
  };
    ...
    <div className="max-w-4xl mx-auto px-4 py-12">
      ...
      <div className="max-w-4xl mx-auto">
        <div className="bg-app-card border border-app-border rounded-[40px] p-8 md:p-12 shadow-2xl relative overflow-hidden">
          ...
          {error && step !== 'history' && step !== 'result' && (
            <div className="mb-8 bg-red-500/10 border border-red-500/20 p-4 rounded-2xl flex items-center gap-3 text-red-500 text-sm animate-in slide-in-from-top-2">
          ...
          {step === 'analysis' && (
            <div className="py-20 text-center space-y-8 animate-in fade-in">
              ...
              <div className="flex flex-col items-center gap-4 max-w-xs mx-auto">
                ...
                <p className="text-[10px] uppercase tracking-widest font-bold opacity-40 animate-pulse">{analysisStatus}</p>
              </div>
              ...
              {error && (
                <div className="pt-4">
                  <button 
                    ...
                    className="bg-app-bg border border-red-500/30 text-red-500 font-bold px-8 py-3 rounded-xl hover:bg-red-500/10 transition-all flex items-center gap-2 mx-auto"
                  >
                    <ArrowLeft className="w-4 h-4" /> Go Back & Retry
                  </button>
                </div>
              )}
            </div>
