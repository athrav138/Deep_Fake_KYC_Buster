...
export const generateKYCReport = (data: {
  ...
  const componentBody = [];
  if (data.aadhaarDetails) componentBody.push(['Aadhaar OCR', 'Verified', (data.aadhaarDetails.confidence || 0) + '%']);
  if (data.faceDetails) componentBody.push(['Face Liveness', 'Verified', (data.faceDetails.confidence || 0) + '%']);
  if (data.voiceDetails) componentBody.push(['Voice Auth', 'Verified', (data.voiceDetails.riskLevel || 'N/A') + ' Risk']);
